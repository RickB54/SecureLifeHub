...
const HomeCard = ({ icon: Icon, title, description, onClick, color, className }: HomeCardProps) => {
  ...
  return (
    <div
      className={`bg-gray-800 p-4 rounded-lg flex items-center cursor-pointer transition-colors hover:bg-gray-700 h-full ${className}`}
      onClick={onClick}
    >
    ...
    <div className="pb-20">
      ...
      <div className="space-y-4 mb-8">
        <div className="grid grid-cols-2 gap-3 md:gap-4">
          <HomeCard icon={BarChart} title="Stats" description="Monitor progress" onClick={() => navigate('/stats')} color="blue" />
          <HomeCard icon={Activity} title="Watch" description="Health trends" onClick={() => navigate('/health-dashboard')} color="red" />
        </div>
        <div className="grid grid-cols-2 gap-3 md:gap-4">
          <HomeCard icon={ClipboardList} title="Build" description="Custom plans" onClick={() => navigate('/custom-plans')} color="purple" />
          <HomeCard icon={ClipboardList} title="Saved" description="Your plans" onClick={() => navigate('/custom-plans?showPlans=true')} color="green" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <HomeCard icon={Scale} title="Body Metrics" description="Track body measurements" onClick={() => navigate('/body-metrics')} color="orange" className="w-full" />
          <HomeCard icon={Activity} title="Health Metrics" description="Log your health data" onClick={() => navigate('/2nd-health-metrics')} color="red" className="w-full" />
        </div>
      </div>

      <h2 className="text-xl font-semibold mb-4">Select Workout Type</h2>
      <WorkoutCategoryCards onSelect={(type: string) => navigate(`/create-workout?type=${type}`)} className="mb-6" />

      {/* Primary Action Shortcuts */}
      <div className="space-y-4 mt-8">
        <div className="grid grid-cols-2 gap-4">
          <Button 
            className="w-full h-20 rounded-2xl bg-gradient-to-br from-emerald-600/20 to-teal-500/20 border border-emerald-500/30 text-emerald-100 font-bold p-2" 
          ...
          <Button 
            className="w-full h-20 rounded-2xl bg-gradient-to-br from-blue-600/20 to-indigo-500/20 border border-blue-500/30 text-blue-100 font-bold p-2" 
